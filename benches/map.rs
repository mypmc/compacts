...
macro_rules! gen_bitmap {
    ( 0, 1, $rng:expr ) => {
        { Map::new() }
    };
    ( $size:expr, $end:expr, $rng:expr ) => {
        {
            gen_bitmap!($size, 0, $end, $rng)
        }
    };
...
fn contains(bench: &mut Bencher) {
    ...
    let v1 = gen_bitmap!(SIZE, RANGE1, rng);
...
fn insert(bench: &mut Bencher) {
    let mut rng = rand::thread_rng();
    let mut v1 = gen_bitmap!(SIZE, RANGE1, rng);
...
fn and(bench: &mut Bencher) {
    let mut rng = rand::thread_rng();
    let v1 = gen_bitmap!(SIZE, RANGE1, rng);
    let v2 = gen_bitmap!(SIZE, RANGE2, rng);
...
fn rank_large(bench: &mut Bencher) {
    let mut rng = rand::thread_rng();
    let v1 = gen_bitmap!(SIZE, RANGE2, rng);
    let i = rng.gen();
    bench.iter(|| v1.rank1(i));
}
