...
impl Rank<u16> for Block {
    ...
    fn rank1(&self, i: u16) -> Self::Count {
        ...
        match *self {
            ...
            Block::Seq64(ref seq) => {
                ...
                let vec = &seq.vector;
                let init = vec.iter().take(q).fold(0, |acc, w| acc + w.count_ones());
                let last = vec.get(q).map_or(0, |w| w.rank1(r));
                init + last
            }
            ...
            Block::Rle16(ref rle) => match rle.search(&i) {
                ...
                Ok(n) => {
                    let r = rle.ranges
                        ...
                        .take(n)
                        .sum::<u32>();
                    (i as u32 - (rle.ranges[n].start as u32)) + r + 1
                }
            },
        }
    }
    ...
    fn rank0(&self, i: u16) -> Self::Count {
        i as Self::Count + 1 - self.rank1(i)
    }
}
    ...
    fn select0(&self, c: u16) -> Option<Self::Index> {
        ...
        match *self {
            ...
            Block::Rle16(ref rle) => {
                ...
                let rank1 = match pos {
                    Err(i) if i == 0 => 0,
                    Err(i) => self.rank1(rle.ranges[i - 1].end),
                    Ok(i) => self.rank1(rle.ranges[i].end),
                } as u16;
                Some(c + rank1)
            }
        }
            ...
            fn $fn(self, that: &Block) -> Self::Output {
                match (self, that) {
                    (this @ &Block::Seq16(..), that @ &Block::Seq16(..)) => {
                        ::pair::$fn(this.iter(), that.iter()).collect()
                    }
